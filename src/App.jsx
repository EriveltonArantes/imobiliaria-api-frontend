...
const apiBaseUrl = "https://imobiliaria-api-275k.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Corretor</li>
          <li>✓ Controle completo de Imovel</li>
          <li>✓ Controle completo de Cliente</li>
          <li>✓ Controle completo de Visita</li>
        </ul>
      </div>
...
function PainelCorretor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "creci", "comissao", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/corretors", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/corretors/" + editId : apiBaseUrl + "/api/corretors";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "creci", "comissao"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/corretors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Corretor")}</div>
            ...
            <div className="item-field"><b>creci</b><span>{item["creci"] ?? "—"}</span></div>
            <div className="item-field"><b>comissao</b><span>{item["comissao"] != null ? Number(item["comissao"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Corretor</h3>
      ...
      <label className="field-label">creci</label>
      <input type="text" value={form["creci"] ?? ""} onChange={e => setForm({...form, creci: e.target.value})} />
...
function PainelImovel({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const [corretorList, setCorretorList] = React.useState([]);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["titulo", "descricao", "tipo", "preco", "area", "quartos", "banheiros", "vagas", "endereco", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/corretors", { headers: authHeaders(token) }).then(r => r.json()).then(setCorretorList).catch(() => {});
  ...
  const editar = (item) => {
    const f = {};
    ["titulo", "descricao", "tipo", "preco", "area", "quartos", "banheiros", "vagas", "endereco", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["corretor", "cliente"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["titulo"] ?? "Imovel")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/imovels/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["titulo"] ?? "Imovel")}</div>
            ...
            <div className="item-field"><b>area</b><span>{item["area"] != null ? Number(item["area"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>quartos</b><span>{item["quartos"] ?? "—"}</span></div>
            <div className="item-field"><b>banheiros</b><span>{item["banheiros"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>corretor</b><span>{item.corretorId ? ((corretorList.find(o => o.id === item.corretorId) || {}).nome ?? ("#" + item.corretorId)) : "—"}</span></div>
      ...
      <input type="number" value={form["banheiros"] ?? ""} onChange={e => setForm({...form, banheiros: e.target.value})} />
      ...
      <select value={form["corretor"] ?? ""} onChange={e => setForm({...form, corretor: e.target.value})}>
        <option value="">Selecione...</option>
        {(corretorList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
...
function PainelCliente({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "tipoInteresse", "orcamento", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "tipoInteresse", "orcamento"].forEach(k => { f[k] = item[k] ?? ""; });
    ["corretor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>email</b><span>{item["email"] ?? "—"}</span></div>
            <div className="item-field"><b>telefone</b><span>{item["telefone"] ?? "—"}</span></div>
            <div className="item-field"><b>tipoInteresse</b><span>{item["tipoInteresse"] ?? "—"}</span></div>
            <div className="item-field"><b>orcamento</b><span>{item["orcamento"] != null ? Number(item["orcamento"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>corretor</b><span>{item.corretorId ? ((corretorList.find(o => o.id === item.corretorId) || {}).nome ?? ("#" + item.corretorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      <label className="field-label">telefone</label>
      <input type="text" maxLength={15} value={form["telefone"] ?? ""} onChange={e => setForm({...form, telefone: formatarTelefone(e.target.value)})} />
      <label className="field-label">tipoInteresse</label>
      <input type="text" value={form["tipoInteresse"] ?? ""} onChange={e => setForm({...form, tipoInteresse: e.target.value})} />
      <label className="field-label">orcamento</label>
...
function PainelVisita({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["clienteId", "imovelId", "corretorId", "dataVisita", "observacoes", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/visitas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const montarCorpo = () => {
    const corpo = { ...form };
    ["cliente", "imovel", "corretor"].forEach(k => {
      corpo[k + "Id"] = corpo[k] ? Number(corpo[k]) : null;
      delete corpo[k];
    });
    // Bug real (2026-06-26): <input type="datetime-local"> envia "YYYY-MM-DDTHH:mm"
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/visitas/" + editId : apiBaseUrl + "/api/visitas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["clienteId", "imovelId", "corretorId", "dataVisita", "observacoes", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["cliente", "imovel", "corretor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/visitas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["observacoes"] ?? "Visita")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/visitas/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["observacoes"] ?? "Visita")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            ...
            <div className="item-field"><b>corretorId</b><span>{item["corretorId"] ?? "—"}</span></div>
            <div className="item-field"><b>dataVisita</b><span>{item["dataVisita"] ? new Date(item["dataVisita"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>cliente</b><span>{item.clienteId ? ((clienteList.find(o => o.id === item.clienteId) || {}).nome ?? ("#" + item.clienteId)) : "—"}</span></div>
            <div className="item-field"><b>imovel</b><span>{item.imovelId ? ((imovelList.find(o => o.id === item.imovelId) || {}).titulo ?? ("#" + item.imovelId)) : "—"}</span></div>
            <div className="item-field"><b>corretor</b><span>{item.corretorId ? ((corretorList.find(o => o.id === item.corretorId) || {}).nome ?? ("#" + item.corretorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Visita</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">corretorId</label>
      <input type="number" value={form["corretorId"] ?? ""} onChange={e => setForm({...form, corretorId: e.target.value})} />
      <label className="field-label">dataVisita</label>
      <input type="datetime-local" value={form["dataVisita"] ?? ""} onChange={e => setForm({...form, dataVisita: e.target.value})} />
      ...
      <select value={form["imovel"] ?? ""} onChange={e => setForm({...form, imovel: e.target.value})}>
        <option value="">Selecione...</option>
        {(imovelList || []).map(o => (<option key={o.id} value={o.id}>{o.titulo ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">corretor</label>
      <select value={form["corretor"] ?? ""} onChange={e => setForm({...form, corretor: e.target.value})}>
        <option value="">Selecione...</option>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Imobiliaria<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Corretor" ? " active" : "")} onClick={() => setAba("Corretor")}><span className="nav-ico">🗂️</span>Corretor</button>
        ...
        <button className={"nav-btn" + (aba === "Visita" ? " active" : "")} onClick={() => setAba("Visita")}><span className="nav-ico">📅</span>Visita</button>
      ...
      <main className="main">
        ...
        {aba === "Corretor" && <PainelCorretor token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      ...
      {aba === "Visita" && <PainelVisita token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
